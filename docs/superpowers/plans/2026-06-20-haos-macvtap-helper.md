...
**Goal:** 实现一个 LazyCat `.lpk` 应用 `macvtap-helper`，开机自动加载 macvtap 内核模块并在需要时重启 lightos 实例，使 HAOS（macvtap/LAN 独立 IP 部署）在物理机冷重启后自动恢复；并提供状态页与两个手动按钮。
...
- 包名：`com.github.moonfruit.macvtap-helper`；subdomain：`macvtap-helper`。
- macvtap 父接口：`enp2s0`。
...
**Files:**
- Create: `macvtap-helper/src/go.mod`
- Create: `macvtap-helper/src/macvtap.go`
- Test: `macvtap-helper/src/macvtap_test.go`
...
`macvtap-helper/src/macvtap_test.go`:
```go
package main
...
Run: `cd macvtap-helper/src && go test ./... -run TestMacvtapLoaded -v`
Expected: 编译失败（`MacvtapLoaded` 未定义）/ FAIL。

- [ ] **Step 3: 写 go.mod 与最小实现**

`macvtap-helper/src/go.mod`:
```
...
```bash
git add macvtap-helper/src/go.mod macvtap-helper/src/macvtap.go macvtap-helper/src/macvtap_test.go
git commit -m "feat(macvtap-helper): macvtap /proc/devices detection"
```
...
**Files:**
- Create: `macvtap-helper/src/pkgm.go`
- Test: `macvtap-helper/src/pkgm_test.go`
...
`macvtap-helper/src/pkgm_test.go`:
```go
package main
...
Run: `cd macvtap-helper/src && go test ./... -run TestPkgm -v`
Expected: 编译失败（`Pkgm` 未定义）。

- [ ] **Step 3: 写实现**

`macvtap-helper/src/pkgm.go`:
```go
package main
...
```bash
git add macvtap-helper/src/pkgm.go macvtap-helper/src/pkgm_test.go
git commit -m "feat(macvtap-helper): pkgm 7733 local API client"
```
...
**Files:**
- Create: `macvtap-helper/src/ipc.go`
- Test: `macvtap-helper/src/ipc_test.go`
...
`macvtap-helper/src/ipc_test.go`:
```go
package main
...
Run: `cd macvtap-helper/src && go test ./... -run TestIPC -v`
Expected: 编译失败（未定义）。

- [ ] **Step 3: 写实现**

`macvtap-helper/src/ipc.go`:
```go
package main
...
```bash
git add macvtap-helper/src/ipc.go macvtap-helper/src/ipc_test.go
git commit -m "feat(macvtap-helper): unix-socket JSON IPC between web and agent"
```
...
**Files:**
- Create: `macvtap-helper/src/bootlogic.go`
- Test: `macvtap-helper/src/bootlogic_test.go`
...
`macvtap-helper/src/bootlogic_test.go`:
```go
package main
...
Run: `cd macvtap-helper/src && go test ./... -run TestEnsureMacvtap -v`
Expected: 编译失败（未定义）。

- [ ] **Step 3: 写实现**

`macvtap-helper/src/bootlogic.go`:
```go
package main
...
```bash
git add macvtap-helper/src/bootlogic.go macvtap-helper/src/bootlogic_test.go
git commit -m "feat(macvtap-helper): boot logic (load-if-absent then restart lightos)"
```
...
**Files:**
- Create: `macvtap-helper/src/constants.go`（无 tag，公共常量）
- Create: `macvtap-helper/src/agent.go`（`//go:build linux`，netlink + RealActions + RunAgent）
- Create: `macvtap-helper/src/agent_other.go`（`//go:build !linux`，RunAgent stub）
- Modify: `macvtap-helper/src/go.mod`（加 netlink 依赖）
- Create: `macvtap-helper/src/go.sum`（`go mod tidy` 生成）
...
`macvtap-helper/src/constants.go`:
```go
package main
...
`macvtap-helper/src/agent_other.go`:
```go
//go:build !linux
...
```bash
git add macvtap-helper/src/constants.go macvtap-helper/src/agent.go macvtap-helper/src/agent_other.go macvtap-helper/src/go.mod macvtap-helper/src/go.sum
git commit -m "feat(macvtap-helper): agent mode (netlink macvtap + lightos restart + ipc server)"
```
...
**Files:**
- Create: `macvtap-helper/src/web.go`
- Test: `macvtap-helper/src/web_test.go`
...
`macvtap-helper/src/web_test.go`:
```go
package main
...
Run: `cd macvtap-helper/src && go test ./... -run TestWeb -v`
Expected: 编译失败（未定义）。

- [ ] **Step 3: 写实现**

`macvtap-helper/src/web.go`:
```go
package main
...
```bash
git add macvtap-helper/src/web.go macvtap-helper/src/web_test.go
git commit -m "feat(macvtap-helper): web status page with manual buttons"
```
...
**Files:**
- Create: `macvtap-helper/src/main.go`
- Create: `macvtap-helper/lzc-build.yml`
- Create: `macvtap-helper/Makefile`
- Create: `macvtap-helper/.gitignore`

**Interfaces:**
- Consumes: `RunAgent`（Task5）、`RunWeb`（Task6）。
- Produces: 二进制 `macvtap-helper`，支持 `-agent` 与 `-web -listen <addr>`。

- [ ] **Step 1: 写 main.go**

`macvtap-helper/src/main.go`:
```go
package main
...
Run:
```bash
cd macvtap-helper/src && go test ./... && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o /tmp/macvtap-helper-bin .
```
Expected: 测试全 PASS；`/tmp/macvtap-helper-bin` 生成。

- [ ] **Step 3: 写 lzc-build.yml**

`macvtap-helper/lzc-build.yml`:
```yaml
buildscript: CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go -C src build -o ../dist/macvtap-helper
...
```bash
git add macvtap-helper/src/main.go macvtap-helper/lzc-build.yml macvtap-helper/Makefile macvtap-helper/.gitignore
git commit -m "feat(macvtap-helper): main dispatch + lpk build config"
```
...
**Files:**
- Create: `macvtap-helper/lzc-manifest.yml`
- Create: `macvtap-helper/package.yml`
- Create: `macvtap-helper/lzc-icon.png`（从仓库现有图标复制占位）

**Interfaces:**
- Consumes: 二进制路径 `/lzcapp/pkg/content/macvtap-helper`、socket `/lzcapp/var/ipc/agent.sock`。

- [ ] **Step 1: 写 lzc-manifest.yml**

`macvtap-helper/lzc-manifest.yml`:
```yaml
application:
...
`macvtap-helper/package.yml`:
```yaml
package: com.github.moonfruit.macvtap-helper
version: 0.0.1
name: HAOS Helper
...
Run: `cp router/lzc-icon.png macvtap-helper/lzc-icon.png`
Expected: 文件存在。

- [ ] **Step 4: lint 校验清单**

Run: `cd macvtap-helper && lzc-cli project lint`
...
```bash
git add macvtap-helper/lzc-manifest.yml macvtap-helper/package.yml macvtap-helper/lzc-icon.png
git commit -m "feat(macvtap-helper): lpk manifest (web frontend + host-net agent) + metadata"
```
...
- 安装 `macvtap-helper` 应用（`cd macvtap-helper && make install`），它负责开机加载 macvtap 并在缺失时重启 lightos；
...
Run: `cd macvtap-helper && make && make install`
...
```bash
ssh 192.168.50.11 'find /sys/fs/cgroup -path "*macvtap-helper*" -name cgroup.procs | head; \
  for f in $(find /sys/fs/cgroup -path "*macvtap-helper*" -name cgroup.procs); do for p in $(cat $f); do tr "\0" " " </proc/$p/cmdline; echo; done; done'
```
Expected: 看到前端 `-web` 进程与 agent `-agent` 进程；因部署时 macvtap 已加载 → agent 日志应为 no-op（未重启 lightos，HAOS 未被打断——确认 HAOS qemu pid 未变）。

- [ ] **Step 3: 访问状态页**

打开 `https://macvtap-helper.dkmooncat.heiyu.space/`，确认显示 "macvtap 模块: 已加载"、实例状态码、两个按钮。
...
将集成验证结果补记到 `macvtap-helper/README.md` 或 spec 末尾。
```bash
git add -A && git commit -m "docs(macvtap-helper): record integration verification results"
```
